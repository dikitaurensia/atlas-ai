// AHP weights — Section 2.2.2 (Timor & Sipahi 2005; Tzeng et al. 2002)
const AHP_WEIGHTS = {
  traffic:      0.30,
  competition:  0.25,
  accessibility: 0.20,
...
// Profit margin per kategori — Analog Store Method Section 4.2
// Estimasi awal 20–35% dari data internal ESB; TODO: validasi per sprint Data Science
const MARGIN_KATEGORI = {
  'Kopi & Cafe': 0.30,
  'Ayam Goreng': 0.25,
  'Burger':      0.28,
  'Mie & Bakso': 0.25,
...
// Huff λ — sensitivitas jarak, default λ=2 (dikalibrasi per kota setelah data cukup)
const HUFF_LAMBDA = 2
...
export function generateAnalysis(latlng, category, radius, realData = {}, scale = 'Menengah') {
  const { nearbyCompetitors = null, benchmark = null, footTraffic = null, accessibility = null, demographics = null, osmFoodCount = null } = realData
  ...
  const { competition: rawCompetition, competitionRatio, osmUsed } = calcCompetitionScore(competitorCount, benchmark, radius, osmFoodCount)
  const competition = Math.min(90, Math.max(15, rawCompetition + (SCALE_COMP_ADJ[scale] || 0)))

  // Overall Score — AHP weighted average (Section 2.2.2)
  // Bobot dimensi yang null diredistribusi secara proporsional ke dimensi yang tersedia
  const overall = calcAHPScore({
    traffic,
    competition,
    accessibility: accessibilityScore,
    population: populationScore,
    purchasePower: purchasePowerScore,
  })

  // Grade thresholds — Section 2.2.3
  let grade, gradeColor
  if (overall >= 80) { grade = 'Sangat Potensial'; gradeColor = '#10B981' }
  else if (overall >= 70) { grade = 'Potensi Bagus';    gradeColor = '#3B82F6' }
  else if (overall >= 60) { grade = 'Cukup Potensial';  gradeColor = '#F59E0B' }
  else                    { grade = 'Kurang Ideal';      gradeColor = '#EF4444' }

  // Analog Store Method — P10/P90 + margin_kategori (Section 4.2)
  const profitData = calcAnalogProfit(competitorList, category)
  ...
  const { profitSource, revenueDataCount, highVariance, margin } = profitData
  ...
  // Huff Gravity Model — capture rate & potensi pelanggan (Section 3)
  const huff = calcHuffModel(radius, competitorList, benchmark, demographics)
  ...
  return {
    ...
    dimensions: [
      { label: 'Traffic Pejalan Kaki', score: traffic,            weight: AHP_WEIGHTS.traffic,       source: footTraffic   ? 'osm'         : 'unavailable' },
      { label: 'Tingkat Persaingan',   score: competition,        weight: AHP_WEIGHTS.competition,   source: osmUsed ? 'db+osm' : (benchmark ? 'db' : 'estimated') },
      { label: 'Aksesibilitas',        score: accessibilityScore, weight: AHP_WEIGHTS.accessibility, source: accessibility ? 'osm'         : 'unavailable' },
      { label: 'Kepadatan Penduduk',   score: populationScore,    weight: AHP_WEIGHTS.population,    source: 'db' },
      { label: 'Daya Beli Area',       score: purchasePowerScore, weight: AHP_WEIGHTS.purchasePower, source: 'db' },
    ],
    profitMin, profitMax, profitMedian, profitSource, margin,
...
// AHP weighted average — bobot null diredistribusi ke dimensi yang tersedia (Section 2.2.2)
function calcAHPScore(scores) {
  const dims = [
    { w: AHP_WEIGHTS.traffic,       s: scores.traffic },
    { w: AHP_WEIGHTS.competition,   s: scores.competition },
    { w: AHP_WEIGHTS.accessibility, s: scores.accessibility },
    { w: AHP_WEIGHTS.population,    s: scores.population },
    { w: AHP_WEIGHTS.purchasePower, s: scores.purchasePower },
  ]
  const available = dims.filter(d => d.s !== null)
  if (available.length === 0) return 0
  const totalWeight = available.reduce((sum, d) => sum + d.w, 0)
  return Math.round(available.reduce((sum, d) => sum + (d.w / totalWeight) * d.s, 0))
}

// Analog Store Method — P10/P90 percentile + margin_kategori (Section 4.2)
// N ≥ 5 wajib; jika kurang, tidak ditampilkan (Section 4.3)
function calcAnalogProfit(competitors, category) {
  const revenues = competitors
    .filter(c => c.revenue_jt != null)
    .map(c => parseInt(c.revenue_jt))
    .sort((a, b) => a - b)

  const N = revenues.length
  const margin = MARGIN_KATEGORI[category] || 0.25

  // Threshold N ≥ 5 (Section 4.3 — kriteria seleksi outlet analog)
  if (N < 5) {
    return { profitMin: null, profitMax: null, profitMedian: null, profitSource: N > 0 ? 'insufficient' : 'none', revenueDataCount: N, highVariance: false, margin }
  }

  // Nearest-rank percentile (IQR P25-P75 — lebih representatif dari P10-P90 untuk N kecil)
  const p25 = revenues[Math.max(0, Math.ceil(0.25 * N) - 1)]
  const p75 = revenues[Math.min(N - 1, Math.ceil(0.75 * N) - 1)]
  const median = revenues[Math.floor((N - 1) / 2)]

  const profitMin = Math.round(p25 * margin)
  const profitMax = Math.round(p75 * margin)
  const profitMedian = Math.round(median * margin)

  // Flag volatilitas tinggi jika SD > 60% dari mean (AC2.5)
  let highVariance = false
  if (N >= 2) {
    const mean = revenues.reduce((a, b) => a + b, 0) / N
    const variance = revenues.reduce((a, b) => a + (b - mean) ** 2, 0) / N
    highVariance = mean > 0 && Math.sqrt(variance) / mean > 0.6
  }

  return { profitMin, profitMax, profitMedian, profitSource: 'analog', revenueDataCount: N, highVariance, margin }
}

// Huff Gravity Model — capture rate pelanggan (Section 3.2–3.3)
// Pᵢⱼ = (Aⱼ / Tᵢⱼ^λ) / Σₖ (Aₖ / Tᵢₖ^λ)
// T_new = radius × 0.4 (representasi rata-rata jarak pelanggan ke toko baru di pusat catchment)
function calcHuffModel(radius, competitors, benchmark, demographics) {
  if (!demographics) return null

  const popRadius = Math.round(demographics.population_density * Math.PI * Math.pow(radius / 1000, 2))

  // Attractiveness proxy: rata-rata revenue benchmark (jt)
  const benchmarkAvg = benchmark
    ? (parseFloat(benchmark.min_jt) + parseFloat(benchmark.max_jt)) / 2
    : 30

  // Representasi jarak rata-rata pelanggan ke toko baru (40% dari radius)
  const T_new = Math.max(10, radius * 0.4)

  const newStoreGravity = benchmarkAvg / Math.pow(T_new, HUFF_LAMBDA)

  // Gravitasi setiap kompetitor
  const competitorsInRadius = competitors.filter(c => parseFloat(c.distance_m) > 0)
  const competitorGravity = competitorsInRadius.reduce((sum, c) => {
    const attr = c.revenue_jt != null ? parseInt(c.revenue_jt) : benchmarkAvg
    const dist = Math.max(1, parseFloat(c.distance_m))
    return sum + attr / Math.pow(dist, HUFF_LAMBDA)
  }, 0)

  const totalGravity = newStoreGravity + competitorGravity
  const captureProb = totalGravity > 0 ? newStoreGravity / totalGravity : 1

  const capturedPopulation = Math.round(captureProb * popRadius)

  return {
    captureRate: Math.round(captureProb * 1000) / 10,  // persentase, 1 desimal
    popRadius,
    capturedPopulation,
    lambda: HUFF_LAMBDA,
    competitorCount: competitorsInRadius.length,
  }
}

function calcCompetitionScore(actualCount, benchmark, radius, osmFoodCount) {
  // DB score — persaingan kategori spesifik (sinyal utama)
  let dbScore, competitionRatio
  if (!benchmark) {
    dbScore = Math.min(90, Math.max(15, Math.round(90 - actualCount * 8)))
    ...
    dbScore = Math.min(90, Math.max(15, Math.round(90 - ratio * 37.5)))
    competitionRatio = ratio
  }

  // OSM score — kejenuhan F&B umum di area (sinyal sekunder, radius-normalized)
  // density 5/km² → 84, density 20/km² → 66, density 50/km² → 30, density 70+/km² → 15
  if (osmFoodCount != null && osmFoodCount > 0) {
    const areakm2 = Math.PI * Math.pow(radius / 1000, 2)
    const density  = osmFoodCount / areakm2
    const osmScore = Math.min(90, Math.max(15, Math.round(90 - density * 1.2)))
    // Blend: 75% DB (kategori langsung) + 25% OSM (saturasi pasar umum)
    const competition = Math.round(dbScore * 0.75 + osmScore * 0.25)
    return { competition, competitionRatio, osmUsed: true }
  }

  return { competition: dbScore, competitionRatio, osmUsed: false }
}
...
function getRecommendation(score, category, competitorCount, competitionRatio, traffic, footTraffic, demographics, scale = 'Menengah') {
  ...
  // Grade thresholds selaras dengan AHP Section 2.2.3 (80/70/60)
  if (score >= 80) {
