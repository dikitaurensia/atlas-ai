...
export default function Header({ historyCount, onRiwayatClick, onLocationSearch, theme, onToggleTheme }) {
  ...
  return (
    <header style={s.header}>
      ...
      <div style={s.brand}>
        ...
        {user?.bisnis_name && (
          <div style={s.workspace}>
            <div style={s.wsDot} />
            <span style={s.wsName}>{user.bisnis_name}</span>
          </div>
        )}
      </div>
      ...
      {/* Actions */}
      <div style={s.actions}>
        <button style={s.iconBtn} onClick={onToggleTheme} title={theme === 'dark' ? 'Mode terang' : 'Mode gelap'}>
          {theme === 'dark' ? (
...
const s = {
  ...
  clearBtn: {
    ...
    background: 'var(--sb-card)', border: 'none',
  ...
  avatarRing: {
    ...
    background: 'linear-gradient(135deg,var(--accent),#6B8FC9)',
  },
  avatarInner: {
